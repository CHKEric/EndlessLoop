...
#start Module:
def Start():
	forward = easygui.buttonbox('说在前面：本程序灵感来源于Bilibili-GoldenEggs作品《无尽循环》，一些图片来源于 《无尽循环》，该程序仅用于娱乐和搞笑。有Bug很正常',choices=['好'],title=['Start'])
	...
	def End():
		...
		if end <= 1:
				...
				if q9_3 == '告诉，真相？':
					...
					pygame.mixer.music.play(start=0.0)
					q9_4 = easygui.buttonbox('你奋力一跳，飞速下降，你掉入下方的湖中，湖旁是一次又一次被击杀的“救兵”的头颅。你开始沉入水中，头套让你喘不过气来……恍惚间，你已醒了。你从床上起来，感觉循环好像一个梦。窗外阵阵鸟叫声，你看表，今天是20XX年6月1日，天气晴。你再次来到了那个时间表的位置，藤蔓已然消失，你进了洞，只见那副墙上，赫然写着：第四循环体……【真结局】 【BGM：洛天依-〇】',choices=['第一循环(通关奖励：任意选）','第二循环','第三循环','彩蛋'],title=['GoodEnd'])
					pygame.mixer.music.stop()
					if q9_4 == '第一循环':
						First.Q1()
					elif q9_4 == '第二循环':
...
class Second():
	...
	def Q6():
		...
		if q6 == '射死他！（%20几率）':
			...
			Exit()
	def Q7():
		q7 = easygui.buttonbox('他跳下去了！你十分焦急，跟着他一跳，没想到，传送门神秘的消失了，你直直坠下，掉入了一地洞。你走过一弯又一弯，看到了另一面墙。那，就是所谓的‘时间表’。你毛骨悚然，恍然大悟，你决定',choices=['当救兵'],title=['3-Q7'],image=['Image/sjb-3.png'])
		ran = random.randint(0,9)
		if q7 == '当救兵':
			 Third.Q8()
		elif q7 == 'Image/sjb-3.png':
			Second.Q7()
		else:
			Exit()
...
class Third():
	...
	def surprise():
			surpr=easygui.buttonbox('‘大人，这是我再次策划的好戏。’你到了一个未知的地方,看见两个人在讲话。‘A，B，和C三个人在这个地图里，相杀相救，被假的诅咒和"真"的时间表所误导，无限循环永无止境，有1,2,3循环……’‘这保险吗？你亲爱的演员又出来了。’‘欧，大人，这个传送门可能有一点故障，我很快就修好。（你被传送回初始循环）【恭喜触发彩蛋结局】【BGM：洛天依——〇】',choices=['继续'],title=['彩蛋'])
			if surpr == '继续':
				First.Q1()
			else:
				Exit()
	def Q4():
		...
		if q4 == '砍他！（%20几率）':
			...
			Exit()
	def Q8():
		q8 = easygui.buttonbox('你在原来的地方放下了那个告示牌，上面写‘别回头’。他还是跟上来了。你将他引到了一片平地，丢下了你珍藏的红宝石，静候他上门。',choices=['引诱'],title=['3-Q8'])
		ran = random.randint(0,9)
		if q8 == '引诱':
			 	Third.End()
		else:
